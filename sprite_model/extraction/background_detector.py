...
Independent module for detecting background colors in sprite sheets.
Used for applying transparency to sprites extracted from solid-background images.

This module is standalone and has no dependencies on the main SpriteModel.
"""
...
def _detect_color_key_mask(img_array: np.ndarray, debug_log: list) -> Optional[Tuple[np.ndarray, np.ndarray, int]]:
    """
...
def _test_color_key_background(img_array: np.ndarray, background_color: np.ndarray, tolerance: int, debug_log: list) -> Optional[Tuple[np.ndarray, float]]:
    """
