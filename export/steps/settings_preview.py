...
class ExportSettingsPreviewStep(WizardStep):
    ...
    def _create_preview_panel(self) -> QWidget:
        ...
        # Preview hint
        hint_label = QLabel("💡 Tip: Use mouse wheel to zoom, drag to pan")
        hint_label.setStyleSheet("color: #17a2b8; font-size: 11px;")
    ...
    def _create_naming_card(self) -> SettingsCard:
        ...
        self.base_name_edit.setMinimumWidth(200)
        self.base_name_edit.setPlaceholderText("Base filename for frames")
    ...
    def _create_style_card(self) -> SettingsCard:
        ...
        self.spacing_spin.setMinimumWidth(100)
        self.spacing_spin.setToolTip("Space between sprites in pixels")
        self.spacing_spin.valueChanged.connect(self._on_setting_changed)
        
        card.add_row("Spacing:", self.spacing_spin, "between sprites")
        
        # Background
        self.bg_combo = QComboBox()
        self.bg_combo.setMinimumWidth(150)
    ...
    def _update_preview(self):
        ...
        if not self._current_preset:
            self.preview_view.update_preview(QPixmap(), "Select an export type to see preview")
            return
        
        if not self._sprites:
            self.preview_view.update_preview(QPixmap(), "Loading sprites...")
            return
    ...
    def _update_export_summary(self):
        ...
        if output_dir:
            import os
            # Show abbreviated path for better clarity
            if len(output_dir) > 40:
                dir_parts = output_dir.split(os.sep)
                if len(dir_parts) > 3:
                    short_path = os.sep.join(['...'] + dir_parts[-2:])
                else:
                    short_path = output_dir
            else:
                short_path = output_dir
            self.export_summary_label.setText(f"📁 Export to: {short_path}")
            self.export_summary_label.setToolTip(f"Full path: {output_dir}")
        else:
