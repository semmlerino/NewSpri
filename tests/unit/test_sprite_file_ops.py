...
from sprite_model.sprite_file_ops import FileLoader, FileValidator
...
class TestFileValidatorPathChecks:
    def test_empty_path_is_invalid(self):
        valid, msg = FileValidator().validate_file_path("")
        assert not valid
    ...
    def test_missing_file_is_invalid(self, tmp_path: Path):
        valid, msg = FileValidator().validate_file_path(str(tmp_path / "ghost.png"))
        assert not valid
        assert "does not exist" in msg

    def test_directory_is_invalid(self, tmp_path: Path):
        valid, msg = FileValidator().validate_file_path(str(tmp_path))
        assert not valid
    ...
    def test_unsupported_format_is_invalid(self, tmp_path: Path):
        ...
        valid, msg = FileValidator().validate_file_path(str(bad))
        assert not valid
    ...
    def test_empty_file_is_invalid(self, tmp_path: Path, qapp):
        ...
        empty.write_bytes(b"")
        valid, msg = FileValidator().validate_file_path(str(empty))
        assert not valid
        assert "empty" in msg.lower()

    def test_valid_png_passes(self, tmp_path: Path, qapp):
        good = _write_real_png(tmp_path / "good.png")
        valid, msg = FileValidator().validate_file_path(str(good))
        assert valid, msg
        assert msg == ""
...
class TestFileLoader:
    ...
    def test_loads_valid_png(self, tmp_path: Path, qapp):
        ...
        success, pixmap, msg = FileLoader().load_sprite_sheet(str(good))
        assert success, msg
    ...
    def test_missing_file_returns_validation_error(self, tmp_path: Path, qapp):
        success, pixmap, msg = FileLoader().load_sprite_sheet(str(tmp_path / "ghost.png"))
        assert not success
    ...
    def test_unsupported_extension_returns_validation_error(self, tmp_path: Path, qapp):
        bad = tmp_path / "data.txt"
        bad.write_text("not an image")
        success, pixmap, msg = FileLoader().load_sprite_sheet(str(bad))
        assert not success
        assert pixmap is None
    ...
    def test_corrupt_image_returns_load_failure(self, tmp_path: Path, qapp):
        ...
        success, pixmap, msg = FileLoader().load_sprite_sheet(str(broken))
        assert not success
        assert pixmap is None
