...
from export.dialogs.export_mode_ui_registry import _UI_MODE_SPECS, _get_ui_mode_spec
...
def test_ui_mode_registry_covers_every_export_mode():
    assert set(_UI_MODE_SPECS) == set(ExportMode)


def test_segments_sheet_uses_sheet_settings_ui():
    sheet_spec = _get_ui_mode_spec(ExportMode.SPRITE_SHEET)
    segments_spec = _get_ui_mode_spec(ExportMode.SEGMENTS_SHEET)
